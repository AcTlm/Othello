...
for i in range(1,6):
    for j in range(1,6):
        j1=Joueur("AlphaBeta",1,i) #initialise deux joueurs j1 et j2 respectivement noirs et blancs
        j2=Joueur("AlphaBeta",2,j)
        print("Bienvenue à Othello le jeu trop rigolo (version statistiques avancées)")
        nombre_repetitions = 15
        ...
        for k in range(2):
            lignes=[]
            try:
                for l in range(nombre_repetitions):
                    print(f"le joueur {description_joueur_1} vs le joueur {description_joueur_2} au {l}eme tour de boucle de la {k}eme execution")
                    plat=Plateau()
                    t1=time.time()
                    ...
                    lignes.append([l,scores_finaux[0],scores_finaux[1],"egalité" if scores_finaux[0]==scores_finaux[1] else equivalence_joueur_description[scores_finaux[2]],time.time()-t1])
            except Exception as e:
                pass
            with open(f"{description_joueur_1}_vs_{description_joueur_2}_{nombre_repetitions}_repetitions_{k}.csv",'w',newline="") as f:
