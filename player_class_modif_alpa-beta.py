...
class Joueur:
    ...
    def get_move(self,plateau,adversaire):        
        ...
        if self.type_joueur=="AlphaBeta":
            vraie_profondeur=min(self.profondeur,64-len(plateau)-1)
            resultat=alpha_beta(plateau,vraie_profondeur,self,adversaire, -100000, +100000)[0]
            if resultat != None:
                x,y=resultat
            else: #règle le cas de bord du dernier coup
                x,y=random.choice(plateau.liste_coup_valide(self))
...
def alpha_beta(plateau_actuel,profondeur:int,joueur_actuel:Joueur,adversaire:Joueur, alpha, beta):
    #print(f"minmax({'taille du plateau ' + str(len(plateau_actuel))},{'profondeur = '+str(profondeur)},{joueur_actuel},{adversaire})")
    print(plateau_actuel)
    liste_coup=plateau_actuel.liste_coup_valide(joueur_actuel)
    meilleur_coup=[None,None]
    ...
    if joueur_actuel.type_joueur == "AlphaBeta":
        maxEval=-infini
        for coup in liste_coup:
            ...
            print(plateau_copy)
            #print("profondeur =",profondeur,"len = ",len(plateau_copy))
            eval=alpha_beta(plateau_copy,profondeur-1,adversaire,joueur_actuel,alpha,beta)[1]
            ...
            if beta <= alpha:
                break
            print(meilleur_coup,maxEval,profondeur)
        ...
        for coup in liste_coup:
            ...
            if beta <= alpha:
                break
            print(meilleur_coup,minEval,profondeur)
            return [meilleur_coup,minEval,profondeur]